...
//        props.put("mail.smtp.auth", "true");
      ...
      //Первый файл
        ...
        textBodyPart.setContent("Доброго дня, відділення №" + warehouse.getNumber() + " м. " + warehouse.getCity()
        					   + "<br><br>Дозвольте ознайомити вас із Розпорядженням №512 «Про розміщення у відділеннях ТОВ «Нова Пошта» оновлених рекламних матеріалів в межах акції «Сфотографуйте чергу – отримайте промокод на знижку» за посиланням: <a href=\"http://sd.np.ua/sd/operator/#uuid:publish$96617204\">http://sd.np.ua/sd/operator/#uuid:publish$96617204</a> . Згідно цього Розпорядження та Додатку 1 до нього всі Відділення із переліку в Додатку 1 беруть участь в акції «Сфотографуйте чергу – отримайте промокод на знижку»."
        					   + "<br><br>Впродовж п'ятниці <b>22.12.2017</b> на ваше Відділення надійдуть нові рекламні постери на заміну постерів «SMS про чергу». Постер може бути розміром <b>А1 або А2.</b>"
        					   + "<br><br>У вкладенні до цього електронного листа ви знайдете два файли формату png із назвами <b>\""+ warehouse.getCity() + ", №" + warehouse.getNumber() + " - big.png\" та \""+ warehouse.getCity() + ", №" + warehouse.getNumber() + " - small.png\"</b> із зображеннями QR-кодів."
        					   + "<br><br>Вам необхідно роздрукувати той файл з QR-кодом, що відповідає розміру постеру, який отримало Відділення: великий QR-код \""+ warehouse.getCity() + ", №" + warehouse.getNumber() + " - big.png\" для постеру А1, малий QR-код \""+ warehouse.getCity() + ", №" + warehouse.getNumber() + " - small.png\" для постеру А2. На постері в полі для QR-коду додатково розміщено підказку. Далі вам потрібно вирізати та вклеїти QR-код у відповідне поле на постері."
        					   + "<br><br>Також в Додатку 1 ви знайдете ваш індивідуальний код для sms із чотирьох цифр. Його необхідно вписати у Додаток 6 (файл формату pdf) та роздрукувати на аркуші «папір А4 самоклейка 8 етикеток (105*74мм)». У разі його відсутності, роздрукувати на звичайному папері. Далі вирізати його та наклеїти на постер на спеціально відведене місце на постері."
        					   + "<br><br>Якщо ви отримали цього електронного листа, але <b>не знайшли себе у переліку з Додатку 1</b> до Розпорядження - цей лист надіслано вам помилково. Ваше Відділення не бере участі в акції, вам не потрібно здійснювати жодних дій за згаданою акцією."
        					   + "<br><br>Якщо ви знайшли себе в переліку з Додатку 1 до Розпорядження, але з якихось причин <b>не отримали постера</b> - напишіть про це Валерії Кучер (менеджер з розробки цифрових продуктів) на kucher.vg1@novaposhta.ua та отримайте від неї подальші інструкції."
        					   + "<br><br>Бажаємо наснаги!"
        					   + "<br><br>Валерія Кучер,\nClient Mobile Application Nova Poshta", "text/html; charset=utf-8");
